...
template <typename WalkType> struct update_delta {
  template <typename NT> static void apply(WalkType, NT) {}
};
...
struct update_delta<GaussianBallWalk::Walk<Polytope, RandomNumberGenerator>> {
  ...
  static void
  apply(GaussianBallWalk::Walk<Polytope, RandomNumberGenerator> walk,
        NT delta) {
...
// Gaussian Anealling
//
// Implementation is based on algorithm from paper "A practical volume
// algorithm", Springer-Verlag Berlin Heidelberg and The Mathematical
// Programming Society 2015 Ben Cousins, Santosh Vempala
//
// Compute the first variance a_0 for the starting gaussian

template <typename Polytope, typename NT>
void get_first_gaussian(Polytope &P, NT const &frac, NT const &chebychev_radius,
                        NT const &error, std::vector<NT> &a_vals) {
  // if tol is smaller than 1e-6 no convergence can be obtained
  // when float is used
  ...
  if (i == maxiter) {
#ifdef VOLESTI_DEBUG
    std::cout << "c [volesti] Cannot obtain sharp enough starting Gaussian"
              << std::endl;
#endif
    return;
  }
...
template <typename RandomPointGenerator, typename Polytope, typename Point,
          typename NT, typename RandomNumberGenerator>
NT get_next_gaussian(Polytope &P, Point &p, NT const &a, const unsigned int &N,
                     const NT &ratio, const NT &C,
                     const unsigned int &walk_length,
                     RandomNumberGenerator &rng) {

  NT last_a = a;
  NT last_ratio = 0.1;
  // k is needed for the computation of the next variance
  // a_{i+1} = a_i * (1-1/d)^k
  NT k = 1.0;
  ...
  for (auto it = fn.begin(); it != fn.end(); ++it) {
    if (!std::isfinite(*it)) {
      throw std::runtime_error("Distance must be a real number");
    }
  }
  ...
  while (!done) {
    assert(std::isfinite(last_a) && "last_a must consist of real numbers");
    ...
    for (auto pit = randPoints.begin(); pit != randPoints.end();
      ...
      auto fnit_numerator = eval_exp(*pit, new_a);
      auto fnit_denominator = eval_exp(*pit, last_a);
      assert(std::isfinite(fnit_numerator) &&
             "fnit_numerator must consist of real numbers");
      assert(std::isfinite(fnit_denominator) &&
             "fnit_denominator must consist of real numbers");
      assert(fnit_denominator != 0 && "fnit_denominator must not be zero");
      *fnit = fnit_numerator / fnit_denominator;
      assert(std::isfinite(*fnit) && "fnit must consist of real numbers");
    }
    std::pair<NT, NT> mv = get_mean_variance(fn);

    // Compute a_{i+1}
    if (mv.second / (mv.first * mv.first) >= C ||
        mv.first / last_ratio < 1.0 + tol) {
      if (k != 1.0) {
        k = k / 2;
      }
      done = true;
    ...
    std::cout << "c [volesti] k = " << k << " mv " << mv.first << " "
              << mv.second << std::endl;
    last_ratio = mv.first;
  }
...
template <typename WalkType, typename RandomPointGenerator, typename Polytope,
          typename NT, typename RandomNumberGenerator>
void compute_annealing_schedule(Polytope &P, NT const &ratio, NT const &C,
                                NT const &frac, unsigned int const &N,
                                unsigned int const &walk_length,
                                NT const &chebychev_radius, NT const &error,
...
#ifdef VOLESTI_DEBUG
  std::cout << "c [volesti] first gaussian computed" << std::endl;
#endif
...
#ifdef VOLESTI_DEBUG
  std::cout << "c [volesti] Computing the sequence of gaussians..."
            << std::endl;
#endif
  ...
  while (true) {
    ...
    update_delta<WalkType>::apply(
        walk, 4.0 * chebychev_radius /
...
template <typename NT> struct gaussian_annealing_parameters {
  gaussian_annealing_parameters(unsigned int d)
      : frac(0.1), ratio(NT(1) - NT(1) / (NT(d))), C(NT(2)),
        N(500 * ((int)C) + ((int)(d * d / 2))), W(6 * d * d + 800) {}
...
template <typename WalkTypePolicy, typename Polytope,
          typename RandomNumberGenerator

          >
double volume_cooling_gaussians(Polytope &Pin, RandomNumberGenerator &rng,
  ...
  typedef typename Polytope::VT VT;
  typedef
      typename WalkTypePolicy::template Walk<Polytope, RandomNumberGenerator>
          WalkType;
  ...
  // Move the chebychev center to the origin and apply the same shifting to the
  // polytope
  P.shift(c.getCoefficients());

  // Computing the sequence of gaussians
#ifdef VOLESTI_DEBUG
  std::cout << "c [volesti] Chebychev Center c: [";
  for (unsigned int i = 0; i < c.dimension(); ++i) {
    std::cout << c[i] << ",";
  }
  std::cout << "] radius: " << radius << std::endl;

  std::cout << "c [volesti] Computing annealing" << std::endl;
  ...
  if (radius == 0.0) {
    std::cout << "c [volesti] radius is zero, forcing to 1.0" << std::endl;
    radius = 1.0;
  }

  std::cout << "c [volesti] computing annealing schedule" << std::endl;
...
#ifdef VOLESTI_DEBUG
  std::cout
      << "c [volesti] All the variances of schedule_annealing computed in = "
      << (double)clock() / (double)CLOCKS_PER_SEC - tstart2 << " sec"
      << std::endl;
  auto j = 0;
...
#ifdef VOLESTI_DEBUG
  std::cout << "c [volesti] volume of the first gaussian = " << vol
            << std::endl;
  std::cout << "c [volesti] computing ratios.." << std::endl;
#endif

  // iterate over the number of ratios
  for (viterator fnIt = fn.begin(); fnIt != fn.end();
    ...
    update_delta<WalkType>::apply(
        walk, 4.0 * radius / std::sqrt(std::max(NT(1.0), *avalsIt) * NT(n)));
...
#ifdef VOLESTI_DEBUG
    std::cout << "c [volesti] ratio " << i << " = " << (*fnIt) / (*itsIt)
              << " N_" << i << " = " << *itsIt << std::endl;
#endif
    vol *= ((*fnIt) / (*itsIt));
  }
  ...
  std::cout << "c [volesti] Total number of steps = " << steps << std::endl;
#endif

  return vol;
}

template <typename WalkTypePolicy = GaussianCDHRWalk,
          typename RandomNumberGenerator =
              BoostRandomNumberGenerator<boost::mt11213b, double>,
          typename Polytope>
double volume_cooling_gaussians(Polytope &Pin, double const &error = 0.1,
                                unsigned int const &walk_length = 1) {
