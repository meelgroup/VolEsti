...
template <typename Point> void print_point(const Point &p) {
  std::cout << "point: (";
...
public:
  ...
  void set_interior_point(Point const &r) {
    ...
    has_ball = true;
    std::cout << "c [volesti->hpolytope] Setting interior point: ["
              << (int)has_ball << "]" << std::endl;
  }
  ...
  std::pair<Point, NT> ComputeInnerBall() {
    ...
    if (!has_ball) {
      ...
      std::tuple<VT, NT, bool> inner_ball =
          max_inscribed_ball(A, b, 500000, tol);
      // std::cout << "c internal method to count inner ball returned radius:
      // "<< std::get<1>(inner_ball) << std::endl; _inner_ball =
      // ComputeChebychevBall<NT, Point>(A, b); // use lpsolve library std::cout
      // << "c lpsolve returned radius: "<< _inner_ball.second << std::endl;
      ...
      if (is_in(Point(std::get<0>(inner_ball))) == 0 ||
        ...
        std::cerr
            ...
            << std::endl;
        std::cerr << "is_in: " << is_in(Point(std::get<0>(inner_ball)))
                  << std::endl;
        std::cerr << "inner_ball radius: " << std::get<1>(inner_ball)
                  << std::endl;
        std::cerr << "is_nan: " << std::isnan(std::get<1>(inner_ball))
                  << std::endl;
        std::cerr << "is_inf: " << std::isinf(std::get<1>(inner_ball))
                  << std::endl;
        std::cerr << "is_inner_point_nan_inf: "
                  << is_inner_point_nan_inf(std::get<0>(inner_ball))
                  << std::endl;
#ifndef DISABLE_LPSOLVE
        _inner_ball =
    ...
    std::cout << "Chebychev centre: [";
    for (int i = 0; i < _inner_ball.first.dimension(); ++i) {
      std::cout << _inner_ball.first[i] << " ";
    }
    std::cout << "]" << std::endl;
    if (is_in(_inner_ball.first) == 0) {
      _inner_ball.first = get_an_inner_point();
      std::cout << "c using alternative point: [";
      for (int i = 0; i < _inner_ball.first.dimension(); ++i) {
        std::cout << _inner_ball.first[i] << " ";
      }
      std::cout << "]" << std::endl;
  ...
  /**
   * @brief Get an inner point of the polytope.
   *
   * This function computes a feasible point within the polytope defined by
   * the constraints A and b. It checks if the computed point is indeed
   * inside the polytope and prints a warning message if it is not.
   *
   * @return A feasible point within the polytope.
   */
  Point get_an_inner_point() const {
    VT meanpoint = compute_feasible_point(A, b);

    if (is_in(meanpoint) == 0)
      std::cout << "Warning: inner point is outside of the polytope"
                << std::endl;
    else {
      std::cout << "Yeye, inner point is inside of the polytope" << std::endl;
    }
    return meanpoint;
  }
  ...
  /**
   * @brief Checks if a given point is inside the H-polytope.
   * @return Returns -1 if the point is inside the H-polytope, otherwise returns
   * 0.
   */
  ...
  std::vector<NT> get_dists(NT const &radius) const {
    std::cout
        << "c [volesti] return for each facet the distance from the origin"
        << std::endl;
    unsigned int i = 0;
  ...
  template <typename update_parameters>
  auto compute_reflection(Point &v, Point &p, update_parameters const &params)
      const -> std::enable_if_t<
                std::is_same_v<MT, Eigen::SparseMatrix<NT, Eigen::RowMajor>> &&
                    !std::is_same_v<update_parameters, int>,
                void> { // MT must be in RowMajor format
  ...
  template <typename update_parameters>
  NT compute_reflection(Point &v, const Point &, DenseMT const &AE,
                        VT const &AEA, NT const &vEv,
                        update_parameters const &params) const {

    Point a((-2.0 * params.inner_vi_ak) * A.row(params.facet_prev));
    VT x = v.getCoefficients();
    NT new_vEv = vEv - (4.0 * params.inner_vi_ak) *
